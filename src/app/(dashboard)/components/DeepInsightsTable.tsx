...
import { Info, Download, Moon, Sun, CircleCheck, CircleX } from 'lucide-react';
...
const mockData: Insight[] = dummyProspects
  .slice(0, 2)
...
export default function DeepInsightsTable() {
  ...
  const scoreRank: Record<Insight['score'], number> = {
    'A+': 3,
    A: 2,
    B: 1,
    C: 0,
  ...
  const downloadCSV = () => {
    ...
    const blob = new Blob([header, ...rows] as BlobPart[], {
      type: 'text/csv',
    });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'deep_insights.csv';
    a.click();
  };
