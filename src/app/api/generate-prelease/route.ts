...
export async function POST(req: NextRequest) {
  try {
    ...
    const bodyFontUrl = 'https://raw.githubusercontent.com/google/fonts/main/apache/arimo/Arimo%5Bwght%5D.ttf';
    const bodyFontBuffer = await fetch(bodyFontUrl)
      .then((res) => res.arrayBuffer())
      .then((buf) => Buffer.from(buf));

    // Fetch signature font (Pacifico)
    const signatureFontUrl = 'https://github.com/google/fonts/raw/main/ofl/pacifico/Pacifico-Regular.ttf';
    ...
    doc
      ...
      .fontSize(12)
      .text(`This Pre-Qualification Form ("Form") is entered into between Grand Oaks Apartments ("Landlord") and ${fullName} ("Applicant") on the date of ${todayDate}.`)
      .moveDown()
      ...
      .text('   This Form serves to reserve an apartment home for the Applicant, subject to the approval of a full application, credit and background screening, and signing of the standard lease contract.')
      .moveDown()

      .text('4. Non-Binding Nature:')
      .text('   This Form does NOT constitute a binding lease agreement. Final lease execution is contingent upon full application approval.')
      .moveDown()

      .text('5. Application Fee:')
      .text('   Applicant agrees to pay all applicable application and administrative fees at the time of completing the formal lease process.')
      .moveDown()
    ...
    doc
      .font('Signature-Regular')
      .fontSize(26)
      .text(fullName, { underline: true })
      .moveDown(1);
